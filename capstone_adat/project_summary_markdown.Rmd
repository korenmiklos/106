...
It showes the list of districts where each party over-performed or under-performed compared to the predictions. The green line and the dotted red lines show the predicted values (so over and underprediction is clearly visible)
```{r warning=FALSE}
### OEVK predictions
...
TBD - Mention AIC score for Fidesz and Jobbik
```{r warning=FALSE}
...
It is likely that analyzing different town sizes separately would improve the effectiveness of this prediction model. Also, since Budapest with 1 million voters counts as 1 data point while all the other towns account for 4.5 million votes in 3150 data points, there is a clear bias towards smaller cities.

TBD - Mention AIC score for Jobbik


### Analyzing outliers - TBD!!

The prediction shows some towns with vastly different vote shares in 2018 compared to 2014. Towns such as x y z are in districts which were Jobbik target seats. As it was shown before, Fidesz performed better than expected in rural seats, meaning that it is likely that they have targeted these towns and OEVKs during the campaign.
```{r warning=FALSE}
...
TBD - analyze over/underperformance
```{r warning=FALSE}
...
This shows the list of constituencies and contituency profile types where parties overperformed or underperformed compared to the expected UNS vote share.
```{r warning=FALSE}
...
data <- read.csv("vote_counts_precincts_2b.csv")
  ...
  #2014 results -party list - TB ADDED
  ...
  #2018 results - add party lists and OEVK
